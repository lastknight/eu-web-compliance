L'intervista al cliente non parte da un foglio bianco, e non parte nemmeno dallo scan: parte dalle gate. Principio fondante: **lo scan tecnico può confermare una presenza, mai un'assenza**. Se il sito non mostra tracce di un CRM non significa che il CRM non esista; la superficie pubblica del sito è una frazione della realtà dei trattamenti (molte aziende hanno sul sito poco o nulla e tutto il trattamento vero altrove).

Il flusso standard è in quattro passi. **Primo: le 16 domande gate di livello 1**, da fare al cliente prima di tutto (cinque minuti, anche al telefono o via email): accendono o spengono intere aree e dicono allo scan dove guardare e quali proprietà esterne esistono (CRM, piattaforma newsletter, app, canali voce). **Secondo: lo scan tecnico**, che parte col perimetro già informato dalle gate e precompila le risposte di livello 0, quelle che una macchina può osservare da sola (CMP, pixel, terze parti, form, lingue, hosting), con l'evidenza allegata. **Terzo: l'intervista vera**, solo sulle aree attive: il cliente conferma le risposte precompilate e riempie le domande di dettaglio di livello 2. **Quarto: la seconda passata di verifica**, che assegna a ogni risposta un verdetto CONFERMATO, SMENTITO o NON VERIFICABILE con l'evidenza a supporto. Le risposte che estendono il perimetro (piattaforma newsletter, CRM, app mobile, PSP) aggiungono proprietà e vendor alla scansione: sui vendor si consultano solo documenti pubblici, sulle proprietà del cliente si eseguono test attivi soltanto con mandato scritto.

Esiste anche la **modalità rapida** (solo URL, zero domande preliminari): lo scan parte alla cieca, propone le risposte alle gate che riesce a osservare e lascia aperte tutte le altre, marcandole "da chiedere". Utile per un triage o una prevendita; per un audit vero il rischio è dimenticare aree intere, perché ciò che non si vede dal sito resta invisibile.
